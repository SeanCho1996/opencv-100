...
# %%
def band_pass_transform(img, mask_ratio_interval=((0.1, 0.1), (0.5, 0.5)), mode="rgb"):
    # generate band pass filter
    # low pass part
    mask_ratio = mask_ratio_interval[0]
    ...
    low_mask = np.pad(se, pad_size)
    # high pass part
    mask_ratio = mask_ratio_interval[1]
    mask_size = [int(img.shape[i] * mask_ratio[i]) for i in range(2)]
    se = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, mask_size)
    pad_size = ((int((img.shape[0] - mask_size[0]) // 2),
                img.shape[0] - int((img.shape[0] - mask_size[0]) // 2) - mask_size[0]),
                (int((img.shape[1] - mask_size[1]) // 2),
                img.shape[1] - int((img.shape[1] - mask_size[1]) // 2) - mask_size[1]))
    high_mask = np.pad(se, pad_size)
    # band pass mask
    mask = np.logical_xor(low_mask, high_mask).astype(np.int)
    ...
    if mode == "rgb":
        ...
        for i in range(3):
            ...
            # filter operation
            fft_band = fft_shift * mask

            # ifft
            ishift = np.fft.ifftshift(fft_band)  #将零频率分量还原      (ishift == f).sum() = 512*512
...
# %%
i = band_pass_transform(img_gs, mode="gs")
# %%
